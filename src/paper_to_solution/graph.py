...
Consumes canonical Question objects (identical representation whether they
came from PDF ingestion or image OCR), routes each through answer
generation, records per-question errors without aborting the whole run.
"""
...
def _solve_node(state: GraphState) -> GraphState:
    ...
    for qd in state.get("questions", []):
        try:
            ...
            errors.append(f"Q{qd.get('number', '?')}: {type(e).__name__}: {e}")
...
def run_graph(questions: list[Question]) -> GraphState:
    """Pass canonical questions through START -> solve -> END."""
    app = build_graph()
    return app.invoke(
